...
- **WHEN** 段落样式不是 Heading 但具有标题特征（大字号、加粗、编号）
- **THEN** 系统应通过启发式算法识别为标题
- **AND** 自动推断标题层级
- **AND** 排除图表标题、单位标注、脚注等非标题内容
...
| 配置项 | 类型 | 默认值 | 描述 |
|-------|------|--------|------|
| `enabled` | bool | True | 是否启用多模态解析，False 时跳过所有模型调用 |
...
| v0.1.0 | 2026-04-29 | 添加手动标题检测、ELK日志、多模态开关 |
